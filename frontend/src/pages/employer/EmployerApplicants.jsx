...
export function EmployerApplicants() {
  ...
  async function load(p = page, q = search, st = statusFilter) {
    setLoading(true);
    try {
      const { data } = await employerApi.getAllApplications({
        page: p, limit: 20,
        ...(q  ? { search: q } : {}),
        ...(st ? { status: st } : {}),
      });
  ...
  return (
    <div>
      {initialLoading ? (
        ...
        <>
          <h1 className="text-3xl font-extrabold tracking-tight md:text-4xl" style={{ color: "var(--text-primary)" }}>Applicants</h1>
          <p className="mt-1 mb-6 text-base" style={{ color: "var(--text-secondary)" }}>
            {total} applicant{total !== 1 ? "s" : ""} across all of your job listings
          </p>
        </>
      )}
