...
export function EmployerShell() {
  ...
  const [jobsNavFocus, setJobsNavFocus] = useState("jobs");
  ...
  useEffect(() => {
    if (location.pathname !== "/employer/jobs") setJobsNavFocus("jobs");
  }, [location.pathname]);
  ...
  const navItems = [
    ...
    { label: "Applicants", to: "/employer/jobs", icon: Users, badge: metrics?.totalApplications, plain: true },
    ...
    <div className="flex min-h-screen" style={{ background: "var(--bg-elev)" }}>
      ...
      >
        ...
        <nav className="flex flex-col gap-1">
          ...
          {navItems.map(({ label, to, icon: Icon, end, badge, plain }) => {
            ...
            if (plain) {
              const isApplicantsShortcut = to === "/employer/jobs";
              const active = isApplicantsShortcut
                && location.pathname === "/employer/jobs"
                && jobsNavFocus === "applicants";
              return (
                <button
                  key={label}
                  type="button"
                  onClick={() => {
                    if (isApplicantsShortcut) setJobsNavFocus("applicants");
                    navigate(to);
                    setSidebarOpen(false);
                  }}
                  className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${active ? "text-white" : "hover:bg-[var(--bg-elev)]"}`}
                  style={{ background: active ? EMP : "transparent", color: active ? "#fff" : "var(--text-secondary)" }}
                >
            ...
            const isJobsShortcut = to === "/employer/jobs";
            ...
            return (
              <NavLink
                ...
                onClick={() => {
                  if (isJobsShortcut) setJobsNavFocus("jobs");
                  setSidebarOpen(false);
                }}
                className={({ isActive }) => {
                  const effectiveActive = isActive && (!isJobsShortcut || jobsNavFocus === "jobs");
                  return `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${effectiveActive ? "text-white" : "hover:bg-[var(--bg-elev)]"}`;
                }}
                style={({ isActive }) => {
                  const effectiveActive = isActive && (!isJobsShortcut || jobsNavFocus === "jobs");
                  return { background: effectiveActive ? EMP : "transparent", color: effectiveActive ? "#fff" : "var(--text-secondary)" };
                }}
              >
                <Icon size={17} className="shrink-0" />
